...
### Code and files that don't really play a role in the pipeline above, but still are somewhat useful/beneficial to archive..:
...
### Files that are not really useful, but still archieved here:

`notebooks_byproduct/total_variation_7*.ipynb`: in the past we didn't have a threshold thingy, but just took the most persistent 7 features with each hyperparameter. 
